...
class Project:
    ...
    def analysis_report(self) -> str:
        ...
        lines.append("- 名称类似“規制マスク / mask / マスク / デバイス / device”的独立可见或可选图层会归入“特殊”选项，例如白色遮罩、全息屏幕、键盘设备效果。")
        lines.append("- 最终输出 = 1 张身体底图 + 0/1 张表情 + 0/1 张红晕 + 0/1 张特殊效果；特殊效果最后叠加，避免被身体服装挡住。")
        return "\n".join(lines)
...
SPECIAL_NAME_HINTS = {
    ...
    "censor",
    "censorship",
    # 独立的前景/特效道具：ミク 的 “デバイス” 就是这种顶层独立图层，
    # 不能当 fixed layer 先画，否则会被身体服装挡住。
    "デバイス",
    "device",
    "端末",
    "モニター",
