...
from .core import DEFAULT_BLUSH_LABEL, DEFAULT_EXPRESSION_LABEL, DEFAULT_SPECIAL_LABEL, Project, ProjectError, detect_cpu_threads
...
APP_TITLE = (
    ...
    "SINFO为衣服与表情对应文件(如无,填JSON/PBD目录),PNG/TLG为预览与合成用的图片文件"
...
class App(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self.special_combo = self._stacked_combo(options, "特殊", self.special_var, self.schedule_preview)
    ...
    def _refresh_scene_options(self) -> None:
        ...
        special_values = [DEFAULT_SPECIAL_LABEL] + [layer.name for layer in scene.special_layers]

        self.body_combo["values"] = body_values
        self.expr_combo["values"] = expr_values
        self.blush_combo["values"] = blush_values
        self.special_combo["values"] = special_values
        ...
        default_special = next((layer.name for layer in scene.special_layers if layer.layer_id == scene.default_special_id), DEFAULT_SPECIAL_LABEL)

        self.body_var.set(default_body)
        self.expr_var.set(default_expr)
        self.blush_var.set(default_blush)
        self.special_var.set(default_special)
    ...
    def _current_selection(self):
        ...
        return self.project.make_selection(
            ...
            special_name=self.special_var.get().strip() or DEFAULT_SPECIAL_LABEL,
        )
